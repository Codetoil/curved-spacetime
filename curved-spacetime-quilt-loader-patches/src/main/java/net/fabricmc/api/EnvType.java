/*
 * Curved Spacetime is a work-in-progress easy-to-use modular simulator for General Relativity.<br>
...
public enum EnvType
{
	 ...
	 * server logic), the data generator logic, and dedicated server logic. It encompasses everything that is available
	 * on the {@linkplain #SERVER server environment type}.</p>
	 */
	CLIENT,
	/**
	 ...
	 * logic, which are all included in the {@linkplain #CLIENT client environment type}. However, the server
	 * environment type has its libraries embedded compared to the client.</p>
	 */
	SERVER,
	CURVED_SPACETIME
}