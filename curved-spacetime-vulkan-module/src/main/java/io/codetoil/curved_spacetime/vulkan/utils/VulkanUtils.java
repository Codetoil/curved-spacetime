...
public class VulkanUtils
{
	...
	public static void vkCheck(int err, String errMsg)
	{
		...
		if (err == Vulkan.VK_ERROR_UNKNOWN())
		{
		...
		if (err == Vulkan.VK_ERROR_VALIDATION_FAILED())
		{
		...
		if (err == Vulkan.VK_ERROR_OUT_OF_POOL_MEMORY())
		{
			errCode = "VK_ERROR_OUT_OF_POOL_MEMORY";
			errMsg = "A pool memory allocation has failed. This must only be returned if no attempt to allocate host " +
		...
		if (err == Vulkan.VK_ERROR_INVALID_EXTERNAL_HANDLE())
		{
			errCode = "VK_ERROR_INVALID_EXTERNAL_HANDLE";
			errMsg = "An external handle is not a valid handle of the specified type.";
		}
		if (err == Vulkan.VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS())
		{
			errCode = "VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS";
			errMsg = "A buffer creation or memory allocation failed because the requested address is not available. A" +
					" shader group handle assignment failed because the requested shader group handle" +
					" information is no longer valid.";
		}
		if (err == Vulkan.VK_ERROR_FRAGMENTATION())
		{
			errCode = "VK_ERROR_FRAGMENTATION";
			errMsg = "A descriptor pool creation has failed due to fragmentation.";
		}
		if (err == Vulkan.VK_PIPELINE_COMPILE_REQUIRED())
		{
			errCode = "VK_PIPELINE_COMPILE_REQUIRED";
			errMsg = "A requested pipeline creation would have required compilation, but the application requested" +
					" compilation to not be performed.";
		}
		if (err == Vulkan.VK_ERROR_NOT_PERMITTED())
		{
			errCode = "VK_ERROR_NOT_PERMITTED";
			errMsg = "The driver implementation has denied a request to acquire a priority above the default priority" +
					" (VK_QUEUE_GLOBAL_PRIORITY_MEDIUM_EXT) because the application does not have sufficient" +
					" privileges.";
		}
		if (err == Vulkan.VK_ERROR_SURFACE_LOST_KHR())
		{
			errCode = "VK_ERROR_SURFACE_LOST_KHR";
			errMsg = "A surface is no longer available.";
		}
		if (err == Vulkan.VK_ERROR_NATIVE_WINDOW_IN_USE_KHR())
		{
			errCode = "VK_ERROR_NATIVE_WINDOW_IN_USE_KHR";
			errMsg = "The requested window is already in use by Vulkan or another API in a manner which prevents it" +
					" from being used again.";
		}
		if (err == Vulkan.VK_SUBOPTIMAL_KHR())
		{
			errCode = "VK_SUBOPTIMAL_KHR";
			errMsg = "A swapchain no longer matches the surface properties exactly, but can still be used to present" +
					" to the surface successfully.";
		}
		if (err == Vulkan.VK_ERROR_OUT_OF_DATE_KHR())
		{
		...
		if (err == Vulkan.VK_ERROR_INCOMPATIBLE_DISPLAY_KHR())
		{
		...
		if (err == Vulkan.VK_ERROR_INVALID_SHADER_NV())
		{
		...
		if (err == Vulkan.VK_ERROR_VIDEO_PROFILE_FORMAT_NOT_SUPPORTED_KHR())
		{
		...
		if (err == Vulkan.VK_ERROR_VIDEO_PROFILE_CODEC_NOT_SUPPORTED_KHR())
		{
		...
		if (err == Vulkan.VK_ERROR_VIDEO_STD_VERSION_NOT_SUPPORTED_KHR())
		{
			errCode = "VK_ERROR_VIDEO_STD_VERSION_NOT_SUPPORTED_KHR";
			errMsg = "The specified video Std header version is not supported.";
		}
		if (err == Vulkan.VK_ERROR_INVALID_DRM_FORMAT_MODIFIER_PLANE_LAYOUT_EXT())
		{
			errCode = "VK_ERROR_INVALID_DRM_FORMAT_MODIFIER_PLANE_LAYOUT_EXT";
		}
		if (err == Vulkan.VK_ERROR_PRESENT_TIMING_QUEUE_FULL_EXT())
		{
			errCode = "VK_ERROR_PRESENT_TIMING_QUEUE_FULL_EXT";
		}
		if (err == Vulkan.VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT())
		{
			errCode = "VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT";
			errMsg = "An operation on a swapchain created with VK_FULL_SCREEN_EXCLUSIVE_APPLICATION_CONTROLLED_EXT" +
		...
		if (err == Vulkan.VK_THREAD_IDLE_KHR())
		{
			errCode = "VK_THREAD_IDLE_KHR";
			errMsg = "A deferred operation is not complete but there is currently no work for this thread to do at" +
					" the time of this call.";
		}
		if (err == Vulkan.VK_THREAD_DONE_KHR())
		{
		...
		if (err == Vulkan.VK_ERROR_COMPRESSION_EXHAUSTED_EXT())
		{
		...
		if (err == Vulkan.VK_INCOMPATIBLE_SHADER_BINARY_EXT())
		{
			errCode = "VK_INCOMPATIBLE_SHADER_BINARY_EXT";
			errMsg = "The provided binary shader code is not compatible with this device.";
		}
		if (err == Vulkan.VK_PIPELINE_BINARY_MISSING_KHR())
		{
			errCode = "VK_PIPELINE_BINARY_MISSING_KHR";
			errMsg = "The application attempted to create a pipeline binary by querying an internal cache, but the" +
					" internal cache entry did not exist.";
		}
		if (err == Vulkan.VK_ERROR_NOT_ENOUGH_SPACE_KHR())
		{
