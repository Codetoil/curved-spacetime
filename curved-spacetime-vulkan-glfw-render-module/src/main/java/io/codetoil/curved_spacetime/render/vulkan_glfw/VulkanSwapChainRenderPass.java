...
public class VulkanSwapChainRenderPass
{
	...
	public VulkanSwapChainRenderPass(VulkanSwapChain swapChain)
	{
		...
		try (MemoryStack stack = MemoryStack.stackPush())
		{
			...
			attachments.get(0).format(swapChain.getVulkanSurfaceFormat().imageFormat())
					.samples(VK14.VK_SAMPLE_COUNT_1_BIT).loadOp(VK14.VK_ATTACHMENT_LOAD_OP_CLEAR)
					.storeOp(VK14.VK_ATTACHMENT_STORE_OP_STORE).initialLayout(VK14.VK_IMAGE_LAYOUT_UNDEFINED)
			...
			VkSubpassDescription.Buffer subPass =
					VkSubpassDescription.calloc(1, stack).pipelineBindPoint(VK14.VK_PIPELINE_BIND_POINT_GRAPHICS)
			...
			subpassDependencies.get(0).srcSubpass(VK14.VK_SUBPASS_EXTERNAL).dstSubpass(0)
					.srcStageMask(VK14.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT)
					.dstStageMask(VK14.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT).srcAccessMask(0)
					.dstAccessMask(VK14.VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT);

			VkRenderPassCreateInfo renderPassInfo =
					VkRenderPassCreateInfo.calloc(stack).sType(VK14.VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO)
							.pAttachments(attachments).pSubpasses(subPass).pDependencies(subpassDependencies);

			LongBuffer lp = stack.mallocLong(1);
			VulkanUtils.vkCheck(
					VK14.vkCreateRenderPass(swapChain.getVulkanLogicalDevice().getVkDevice(), renderPassInfo, null, lp),
	...
	public void cleanup()
	{
		VK14.vkDestroyRenderPass(this.swapChain.getVulkanLogicalDevice().getVkDevice(), this.vkRenderPass, null);
	}
