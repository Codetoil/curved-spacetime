...
public class FFMGLFWRenderModuleConfig implements ModuleConfig
{
	...
	public FFMGLFWRenderModuleConfig(Logger logger)
	{
		this.logger = logger;
	}
	...
	public FFMGLFWRenderModuleConfig load() throws IOException
	{
		Properties props = new Properties();

		try (FileReader reader = new FileReader(FFMGLFWRenderModuleConfig.FILENAME))
		{
			props.load(reader);
		} catch (FileNotFoundException ex)
		{
			logger.log(Level.WARNING, ex, () -> "Could not find config file " + FFMGLFWRenderModuleConfig.FILENAME);
			this.dirty = true;
		}
		...
		} else
		{
			logger.warning("Could not find required key vsync, resetting to default " +
					FFMGLFWRenderModuleConfig.DEFAULT_VSYNC);
			this.vsync = FFMGLFWRenderModuleConfig.DEFAULT_VSYNC;
			this.dirty = true;
		}
	...
	public void save() throws IOException
	{
		...
		try (FileWriter writer = new FileWriter(FFMGLFWRenderModuleConfig.FILENAME))
		{
