...
public class FFMVulkanModuleDependentFFMVulkanGLFWRenderModuleEntrypoint
		implements FFMVulkanModuleDependentModuleInitializer
{

	@Override
	public void onInitialize(FFMVulkanModuleEntrypoint ffmVulkanModuleEntrypoint)
	{
		try
		{
			MainModuleEngine.getInstance().getCurvedSpacetimeLoader()
					...
					.findFirst().orElseThrow().getDependencyModuleTransferQueue().transfer(ffmVulkanModuleEntrypoint);
