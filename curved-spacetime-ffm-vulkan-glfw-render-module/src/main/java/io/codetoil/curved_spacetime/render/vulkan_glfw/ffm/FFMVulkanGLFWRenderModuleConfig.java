...
public class FFMVulkanGLFWRenderModuleConfig implements ModuleConfig
{
	...
	public FFMVulkanGLFWRenderModuleConfig(Logger logger)
	{
		this.logger = logger;
	}
	...
	public FFMVulkanGLFWRenderModuleConfig load() throws IOException
	{
		Properties props = new Properties();

		try (FileReader reader = new FileReader(FFMVulkanGLFWRenderModuleConfig.FILENAME))
		{
			props.load(reader);
		} catch (FileNotFoundException ex)
		{
			this.logger.log(Level.WARNING, ex,
					() -> "Could not find config file " + FFMVulkanGLFWRenderModuleConfig.FILENAME);
			this.dirty = true;
		}
			...
			} catch (NumberFormatException ex)
			{
				this.logger.log(Level.WARNING, ex,
						() -> "Invalid value for key requestedImages: " + requestedImagesPropValue +
								...
								FFMVulkanGLFWRenderModuleConfig.DEFAULT_REQUESTED_IMAGES);
				this.requestedImages = FFMVulkanGLFWRenderModuleConfig.DEFAULT_REQUESTED_IMAGES;
				this.dirty = true;
			}
	...
	public void save() throws IOException
	{
		...
		try (FileWriter writer = new FileWriter(FFMVulkanGLFWRenderModuleConfig.FILENAME))
		{
