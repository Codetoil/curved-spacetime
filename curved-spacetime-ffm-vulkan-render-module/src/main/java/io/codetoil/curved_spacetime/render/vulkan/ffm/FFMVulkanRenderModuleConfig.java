...
public class FFMVulkanRenderModuleConfig implements ModuleConfig
{
	private static final String FILENAME = "config/vulkan-render-module.config";
	private final Logger logger;
	private boolean dirty = false;

	public FFMVulkanRenderModuleConfig(Logger logger)
	{

		this.logger = logger;
	}

	public FFMVulkanRenderModuleConfig load() throws IOException
	{
		@SuppressWarnings("MismatchedQueryAndUpdateOfCollection") Properties props = new Properties();

		try (FileReader reader = new FileReader(FFMVulkanRenderModuleConfig.FILENAME))
		{
			props.load(reader);
		} catch (FileNotFoundException ex)
		{
			this.logger.log(Level.WARNING, ex, () -> "Could not find config file " + FFMVulkanRenderModuleConfig.FILENAME);
			this.dirty = true;
		}
	...
	public void save() throws IOException
	{
		...
		try (FileWriter writer = new FileWriter(FFMVulkanRenderModuleConfig.FILENAME))
		{
