...
public class VulkanGLFWModuleConfig implements ModuleConfig
{
	...
	public VulkanGLFWModuleConfig load() throws IOException
	{
		@SuppressWarnings("MismatchedQueryAndUpdateOfCollection") Properties props = new Properties();

		try (FileReader reader = new FileReader(VulkanGLFWModuleConfig.FILENAME))
		{
			props.load(reader);
		} catch (FileNotFoundException ex)
		{
			Logger.warn(ex, "Could not find config file " + VulkanGLFWModuleConfig.FILENAME);
			this.dirty = true;
		}
	...
	public void save() throws IOException
	{
		...
		try (FileWriter writer = new FileWriter(VulkanGLFWModuleConfig.FILENAME))
		{
