...
- The Closed Jar Variant, which is faster but less extendable. Requires Java 25 to be installed.
- The Closed Native Variant, which is the fastest but least extendable. Does not require Java 25 to be installed, but less Operating Systems are supported. 

The Java Variants (Quilt Variant and Closed Jar Variant) support the following Operating Systems and Architectures, assuming you can get JDK 25 for that version (Not all are tested):
...
The Native Variants (Closed Native Variants) support the following Operating Systems and Architectures:
- Linux on 64-Bit x86 (x86_64/AMD64)
- Linux on 64-Bit ARM (AArch64/ARM64)
- macOS on 64-Bit x86 (x86_64/AMD64)
- macOS on 64-Bit ARM (AArch64/ARM64)
