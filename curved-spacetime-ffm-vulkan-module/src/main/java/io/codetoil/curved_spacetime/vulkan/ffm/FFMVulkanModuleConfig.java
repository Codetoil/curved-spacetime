...
public class FFMVulkanModuleConfig implements ModuleConfig
{
	...
	public FFMVulkanModuleConfig(Logger logger)
	{

		this.logger = logger;
	}

	public FFMVulkanModuleConfig load() throws IOException
	{
		Properties props = new Properties();

		try (FileReader reader = new FileReader(FFMVulkanModuleConfig.FILENAME))
		{
			props.load(reader);
		} catch (FileNotFoundException ex)
		{
			logger.log(Level.WARNING, ex, () -> "Could not find config file " + FFMVulkanModuleConfig.FILENAME);
			this.dirty = true;
		}
		...
		} else
		{
			logger.warning("Could not find required key validation, resetting to default {}" +
					FFMVulkanModuleConfig.DEFAULT_VALIDATE);
			this._validation = FFMVulkanModuleConfig.DEFAULT_VALIDATE;
			this.dirty = true;
		}
		...
		{
			this.preferredDeviceName = FFMVulkanModuleConfig.DEFAULT_PREFERRED_DEVICE_NAME;
		}
