...
public class FFMVulkanModuleVulkanInstance
{
	...
	public FFMVulkanModuleVulkanInstance(FFMVulkanModuleEntrypoint ffmVulkanModuleEntrypoint, Logger logger)
	{
		this.logger = logger;
		...
		// Validation layers
		boolean validation = ((FFMVulkanModuleConfig) ffmVulkanModuleEntrypoint.getConfig()).validation();
		...
		FFMVulkanUtils.vkCheck(Vulkan.vkCreateInstance(instanceInfo, null, pInstance),
	...
	private static MemorySegment createDebugCallback(Logger logger)
	{
		...
		VkDebugUtilsMessengerCreateInfoEXT.messageSeverity(result, FFMVulkanModuleVulkanInstance.MESSAGE_SEVERITY_BITMASK);
		VkDebugUtilsMessengerCreateInfoEXT.messageType(result, FFMVulkanModuleVulkanInstance.MESSAGE_TYPE_BITMASK);
